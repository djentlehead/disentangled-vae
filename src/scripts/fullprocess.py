...
OUTPUT_DIR = "data/processed/fully_processed"
STEPS_PER_BEAT = 24
...
def quantize_midi(midi_obj: pretty_midi.PrettyMIDI, steps_per_beat: int) -> pretty_midi.PrettyMIDI:

    try:
        tempo = midi_obj.get_tempo_changes()[1][0] if len(midi_obj.get_tempo_changes()[1]) > 0 else 120.0
        seconds_per_beat = 60.0 / tempo
        quantization_step_duration = seconds_per_beat / steps_per_beat

        for instrument in midi_obj.instruments: 
            for note in instrument.notes:

                note_start_step = round(note.start / quantization_step_duration)
                note.start = note_start_step * quantization_step_duration
                
                note_end_step = round(note.end / quantization_step_duration) 
                note.end = note_end_step * quantization_step_duration

                if note.start == note.end:
                    note.end += quantization_step_duration
    except Exception as e:
        print(f"Could not quantize MIDI, returning original. Error: {e}")
    
    return midi_obj


print(f"Starting full preprocessing for {len(df)} files...")
...
for index, row in tqdm(df.iterrows(), total=df.shape[0]):
    ...
    try:
        ...
        quantized_midi = quantize_midi(normalized_midi, steps_per_beat=STEPS_PER_BEAT)
        
        quantized_midi.write(output_path)
